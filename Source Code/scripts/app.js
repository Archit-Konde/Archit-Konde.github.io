...
/* ── 14. GITHUB QUADRANT CHART ────────────────────────────────
   Fetches github-stats.json (written nightly by GitHub Actions)
   and renders an SVG quadrant chart matching the GitHub profile
   contribution-type breakdown (commits / PRs / code review / issues).
─────────────────────────────────────────────────────────────── */
(function initGitHubQuadrant() {
  const container = document.getElementById('gh-quadrant');
  const updatedEl = document.getElementById('gh-updated');
  if (!container) return;

  function buildSVG(pct) {
    const W = 380, H = 280, cx = 190, cy = 140, maxR = 100;

    // Each axis point: distance from center proportional to %.
    // Clamp minimum to 3px so even 0% renders a dot at the center.
    const r = {
      commits:      maxR * Math.max(pct.commits,      3) / 100,
      codeReviews:  maxR * Math.max(pct.codeReviews,  3) / 100,
      issues:       maxR * Math.max(pct.issues,        3) / 100,
      pullRequests: maxR * Math.max(pct.pullRequests,  3) / 100,
    };
    ...
    const pts = [
      ...
      [cx,                  cy - r.codeReviews],
      [cx + r.issues,       cy               ],
      [cx,                  cy + r.pullRequests],
    ];

    const poly = pts.map(([x, y]) => `${x.toFixed(1)},${y.toFixed(1)}`).join(' ');
    const dot  = ([x, y]) =>
      `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="4.5" fill="#C9A84C" filter="url(#glow-q)"/>`;

    const pLabel = (n, fallback = '') => n > 0 ? `${n}%` : fallback;
  ...
  <!-- Crosshair -->
  <line x1="0"   y1="${cy}" x2="${W}"  y2="${cy}" stroke="#3e3e42" stroke-width="1"/>
  <line x1="${cx}" y1="0"   x2="${cx}" y2="${H}"  stroke="#3e3e42" stroke-width="1"/>
  ...
  <!-- Commits (left) -->
  ${pct.commits > 0 ? `<text x="82" y="133" text-anchor="end" fill="#C9A84C" font-family="JetBrains Mono,monospace" font-size="11" font-weight="500">${pLabel(pct.commits)}</text>` : ''}
  <text x="82" y="149" text-anchor="end" fill="#858585" font-family="JetBrains Mono,monospace" font-size="11">Commits</text>

  <!-- Code review (top) -->
  ${pct.codeReviews > 0 ? `<text x="${cx}" y="16" text-anchor="middle" fill="#C9A84C" font-family="JetBrains Mono,monospace" font-size="11" font-weight="500">${pLabel(pct.codeReviews)}</text>` : ''}
  <text x="${cx}" y="32" text-anchor="middle" fill="#858585" font-family="JetBrains Mono,monospace" font-size="11">Code review</text>

  <!-- Issues (right) -->
  ${pct.issues > 0 ? `<text x="298" y="133" text-anchor="start" fill="#C9A84C" font-family="JetBrains Mono,monospace" font-size="11" font-weight="500">${pLabel(pct.issues)}</text>` : ''}
  <text x="298" y="${pct.issues > 0 ? '149' : '144'}" text-anchor="start" fill="#858585" font-family="JetBrains Mono,monospace" font-size="11">Issues</text>

  <!-- Pull requests (bottom) -->
  ${pct.pullRequests > 0 ? `<text x="${cx}" y="258" text-anchor="middle" fill="#C9A84C" font-family="JetBrains Mono,monospace" font-size="11" font-weight="500">${pLabel(pct.pullRequests)}</text>` : ''}
  <text x="${cx}" y="274" text-anchor="middle" fill="#858585" font-family="JetBrains Mono,monospace" font-size="11">Pull requests</text>
</svg>`;
  }
